...
6. **Guided Plan tab (YNAB-style, comprehensive)** — a separate entry point (`✨ Guided Plan` in the navbar → `page === "guided"`, component `GuidedPlanWizard`) that does NOT alter the existing landing/detailed-planner. Flow:
   - **Detailed intake questionnaire** — ~40 plain-language multiple-choice questions across 8 sections (About You, Income, Spending, Debt, Savings & Assets, Protection, Goals, Mindset), one at a time with section labels and progress (`GUIDED_QUESTIONS`).
   - **Detailed numbers step** — sectioned, adaptive form capturing income, expenses by category, irregular/annual "true expenses", assets by type, each loan's balance & EMI, insurance cover and goal targets — only asking about what the answers indicated.
   - `buildGuidedData()` maps everything into the existing plan data model.
   - **Tabbed results**: **Budget** (`ZeroBudget` — zero-based "give every rupee a job" with category templates + true-expenses sinking funds and a live To-Assign counter), **Plan** (the full existing `ReportView` — SWOT, goal traffic-lights, Freedom Number, drill-down, PDF), **Debt** (`DebtPayoff` — snowball/avalanche payoff calculator with interest & time saved), and **Reports** (`GuidedReports` — spending breakdown, projected net-worth trend, and YNAB's "Age of Money").
